...
const Event = ({ data, index, parentField = ""  }) => {
  ...
  return (
    <li className="h-event flex text-left" data-tinafield={`${parentField}.${index}`}>
      {data.eventDate && (
        <time className="pt-1 pr-24 sm:pr-12 uppercase text-xs whitespace-nowrap relative border-r border-gray" data-tinafield={`${parentField}.${index}.eventDate`}>
          {data.eventDate}
      ...
      <div className="pl-24 sm:pl-12 text-xl">
        {data.eventName && (
